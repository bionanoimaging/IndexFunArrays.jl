...
for F in generate_functions_expr() 
    ...
    @eval function $(F[1])(::Type{T}, size::NTuple{N, Int};
        ...
        offsets::Tuple{Tuple,Any},   # a version that supports an iterable collection of tuples for offset and scale 
        scales=::Tuple{Tuple,Any},
        dims=ntuple(+, N)) where{N, T} 
        fkt = wrap_zipped(wrap_fkt($(F[2])))
        IndexFunArray(T, (idx->sum(curry(fkt,idx),zip(offsets,scales)), size) 
    end
